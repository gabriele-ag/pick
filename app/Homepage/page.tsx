...
export default function Homepage() {
  ...
  const genres = useMemo(() => {
    ...
    const uniqueGenres = [
      ...new Set(collection.map((curGame: Game) => curGame.genre)),
    ];
  ...
  const handlePick = () => {
    ...
    const pool =
      selectedGenre === "Tutti"
        ? collection
        : collection.filter((game: Game) => game.genre === selectedGenre);
  ...
  return (
    <>
      ...
      <div className={styles.hero}>
        ...
        <p className={styles.descrHero}>
          Ti senti indeciso? Decidi il genere e poi premi su "Scegli casualmente!". 
          Pick ti estrarrà il prossimo gioco dalla tua lista
        </p>
      </div>
      ...
      <section className={styles.sectionChoice}>
        <div className={styles.container}>
          <div className={styles.divChoice}>
            <label className={styles.labelChoice} htmlFor="genre-select">
              Scegli un genere!
            </label>
            <select
              id="genre-select"
            ...
            <button className={styles.buttonPick} onClick={handlePick}>
              {isPicking
                ? "Pick sta decidendo per te..."
                : "Scegli casualmente!"}
            </button>
            
            {winner ? (
              <>
              <h4>Pick ha scelto per te:</h4>
              <div>   
